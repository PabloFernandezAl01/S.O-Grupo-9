...
/**
 * @brief Read data of an opened file
 *
 * Help from FUSE:
 *
 * Read 'size' bytes from the given file into the buffer 'buf', beginning 'offset' bytes into the filehandle
 * Read should return exactly 'size' bytes requested except on EOF or error, otherwise the rest of the data will be substituted with zeroes.
 *
 * @param path file path
 * @param buf buffer where the data read will be stored
 * @param size quatity of bytes to read
 * @param offset offset over the reading
 * @param fi FUSE structure associated to the opened file
 * @return number of bytes read on success and <0 on error
 **/
...
static int my_unlink(const char *path)
{
    ...
    int i=0;
    char* fname=(char*)(path+1); // Convert ffrom const char to char. Y también, quitar el primer character (/) sumando uno al puntero de filename (hace algoritimia de punteros.)
